...
- The key name is the chapter title.
...
current_mapping = {
    "Dimension Eater (1)": [[0, 30], 5],
    "Dimension Eater (2)": [[31, 66], 6],
    "At the Capital": [[67, 96], 7],
    "Entrance Examination Practical Exam - Foundation Arc": [[97, 129], 8],
    "A Bad Omen": [[130], 9]
}
...
for chapter_name, chapter_direction in current_mapping.items():
    page_excerpt = chapter_direction[0]
    chapter_num = be_number(chapter_direction[1])
    final_filename = f"{current_volume:02d}.{chapter_num} - {chapter_name}"
    ...
    for zip_info in zip_contents:
        if zip_info.is_dir():
            continue
        page_number = number(os.path.basename(zip_info.filename))
...
for chapter_info, chapters_file in collected_chapters.items():
    ...
    for chapter in chapters_file:
        zip_target_file.writestr(
            os.path.basename(chapter.filename), zip_data.read(chapter)
        )
